...
// Callback for game input handling
typedef void (*KeyCallback)(unsigned char scancode, bool pressed);
KeyCallback gameKeyCallback = nullptr;

void setKeyboardCallback(KeyCallback callback) {
	gameKeyCallback = callback;
}
...
void handler_clavier(int irq) {
	...
	// Call game input callback if available
	if (gameKeyCallback) {
		if (c < 0x80) {
			// Key press
			gameKeyCallback(c, true);
		} else {
			// Key release (scancode with high bit set)
			gameKeyCallback(c & 0x7F, false);
		}
	}
